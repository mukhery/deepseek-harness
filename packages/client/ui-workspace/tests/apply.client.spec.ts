...
async function bench() {
  ...
  ctx.provide('crewBoard', {
    list: { getSnapshot: () => ({ byWorkspaceId: {} }), subscribe: () => () => {} },
    ensure: ensureCrewBoard,
  } as never)
  const locale = new LocaleRuntime(ctx)
  ctx.provide('locale', locale)
  return {
    ctx, slots: ctx.get('slots') as SlotRegistry, locale, create, startSession, rename,
    insertSessionBefore, open, clear, search, renameSession, binding, fork, ensureCrewBoard,
  }
}
...
describe('ui-workspace apply', () => {
  it('declares the services it drives', () => {
    expect(inject).toEqual(['slots', 'sessions', 'workspaces', 'crewBoard', 'locale'])
  })
  ...
  it('routes browser actions and picker creation to the services', async () => {
    ...
    browser.ensureCrewBoard('ws' as never)
    expect(b.ensureCrewBoard).toHaveBeenCalledWith('ws')
