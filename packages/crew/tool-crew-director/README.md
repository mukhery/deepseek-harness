...
Starts a new continuable subagent (`ctx.subagents.startContinuable`) into a fixed role and records the roster fact (`ctx.crew.hire`). The hired child's identity comes entirely from two `SubagentStartRequest` fields set inline at hire time — **not** from a separate `agent-preset` directory per role: `persona` (fixed prose per role) shadows the deployment persona for that child alone, and `toolFilter.allow` restricts its visible tools to the role's fixed crew tools (`crew_report`/`crew_start_work`/`crew_publish`/`crew_read_pool`, plus `crew_open_ticket` for `strategist` and `crew_verdict` instead of the working-tool set for `reviewer`) unioned with any deployment-configured extras (`Config.roleToolAllow`, e.g. `web_search`/`bash` names, which vary by deployment). This is why only one preset (`crew-director`) exists in this feature — hired members never boot through the preset system at all. The shipped `crew-director` preset's own `roleToolAllow` also gives the `reviewer` role `@deepseek-ai/dsh-tool-session-query`'s `session_search`/`session_trace` family (workspace-authorized, not lineage-authorized), so independent verification can read an assignee's actual session rather than only its self-reported evidence.
...
The generated [`crew_hire`, `crew_open_ticket`, `crew_assign_ticket`, and `crew_board` schemas](../../../docs/tool-catalog.md#deepseek-aidsh-tool-crew-director). The model-facing result stays a compact JSON projection of the underlying `ctx.crew` record(s) (`output.render`), with undefined-valued optional fields omitted (never a raw domain object with `undefined` present). A capable UI without a dedicated crew card instead shows each call's `presentResult` — short human-readable text derived from the same canonical value via `output.presentationMeta` (`crew_board` lists the roster and every ticket one line each), never the raw JSON.
