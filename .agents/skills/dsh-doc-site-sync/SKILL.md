...
- **Change site structure:** update the manifest for ordinary pages; update VitePress configuration only when the existing sidebar or section model cannot express the change.

Never edit or commit `website/.generated/`, `website/.cache/`, or `website/.dist/`. Except for `website/AGENTS.md`, never add Markdown under `website/`; route directories such as `website/api/` are invalid source layouts. Keep generated catalogs under `docs/`, freshness-gate them there, and publish them through the manifest.
...
- `sidebar`: reuse `guide`, `develop`, or `reference` unless the information architecture genuinely needs another collection.
- `section`: reuse an existing section when possible. If adding one, also place it in the `sections` list in [website/docs.ts](../../../website/docs.ts).
...
- Cross-page fragments use the GitHub heading id as their canonical id. If an authored heading emits a different VitePress id, place an explicit `<a id="..."></a>` immediately before it; add generated aliases in the owning generator.
...
If Markdown link checks pass but the site build reports a missing fragment, follow the `verify-doc-site-fragments` source and target paths. Preserve the GitHub id with an explicit alias in authored Markdown or in the owning generator.
